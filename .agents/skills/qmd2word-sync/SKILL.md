---
name: qmd2word-sync
description: Reconcile an editor-returned DOCX into its corresponding Quarto QMD project with qmd2word compare. Use when Word is an editorial authority and an agent must interpret content, structure, styles, comments, tracked changes, fields, tables, images, or generated-output edits, update the right QMD/code/data/config sources, and verify convergence.
---

# Sync edited Word to QMD

Use the edited DOCX as editorial authority for its review round and the matching calling project as the executable representation. Synchronize them semantically; do not seek byte-identical OOXML or infer state from another project that also uses qmd2word.
...
1. Select a fresh, caller-approved output directory and run:
   ...
   ```bash
   qmd2word compare path/to/entry.qmd path/to/editor-returned.docx \
     --output-dir path/to/fresh-comparison
   ```

   If the caller supplies an existing completed bundle, it may be used after verifying its manifest input hashes against the supplied entry QMD and editor DOCX. Never reuse its directory for another comparison.

2. Read `manifest.json`, `source-context.json`, and `diff.json`. Use `report.html` for navigation. Consult the accepted and revision-preserving IR files when a change, comment, style, field, or tracked revision needs more evidence.
3. Group related operations into editorial changes. Anchors and locations aid alignment; they do not dictate source syntax. Interpret insertions, deletions, moves, replacements, structural changes, semantic formatting, comments, and revision evidence together.
...
6. Rerender to a new generated DOCX and run `qmd2word compare` again into another fresh directory. Never use the editor-returned DOCX path as render output.
...
- Do not create a baseline registry, three-way merge, version history, or concurrency protocol inside qmd2word.
