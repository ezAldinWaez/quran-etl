...
This directory is one Quarto project whose editable DOCX deliverables are produced by the headless DocDuet pipeline. The repository owns the report sources, Quran data, and branded Word template; the installed `docduet` package owns the reusable filters, Arabic UI localization, content transplant, post-processing, validation, comparison, and atomic output stages.
...
├── _quarto.yml              # Quarto and DocDuet configuration
├── requirements-docduet.txt # minimal pinned pipeline dependency
└── requirements.txt         # full local report environment
```

## Install and render

Run commands from the repository root. `reports/requirements-docduet.txt` and `skills-lock.json` pin the private `docduet` package and its two agent skills to the `v0.4.0` Git tag so local work and agent workflows use the same release contract. Installation requires GitHub access to the private DocDuet repository. `reports/requirements.txt` includes that minimal file and adds the analytics/Jupyter dependencies used by real reports.
...
The `.quran` mapping lives under `docduet.semantic-styles` in `_quarto.yml` and applies the template’s `Quran` character style to inline spans. Add another semantic mapping only after its named Word style has been deliberately added to the project template.

## Word editorial round trip

The QMD project is the executable representation. A returned editor DOCX is the editorial authority for that review round. Preserve the returned file exactly and compare it in a fresh directory with the exact DOCX sent for review; when that file is unavailable, DocDuet also accepts the matching QMD/project/data version as the baseline:

```bash
python -m quran_etl compare path/to/originally-sent.docx path/to/editor-returned.docx --output-dir reports/output/comparisons/quran-orthography-iteration-1
```

The comparison directory contains the regenerated baseline, an untouched copy of the edited DOCX, structured source context and intermediate representations, `diff.json`, `report.html`, and extracted media. The diff is descriptive evidence; it does not prescribe QMD syntax. Use the repo-local `docduet-sync` skill to interpret the evidence, update the appropriate QMD, code, data, or configuration, rerender, compare into another fresh directory, and record each change as applied, intentionally retained, deferred, or unsupported. Never render over the editor-returned DOCX.
...
The report pipeline is intentionally excluded from GitHub Actions because DocDuet is private and this project does not maintain cross-repository CI credentials. After changing DocDuet configuration, the Word template, localization feature, or report infrastructure, run the small local check:
...
- Use `docduet-author` for report creation or source updates and `docduet-sync` when incorporating an editor-returned Word file.
